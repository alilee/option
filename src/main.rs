...
fn setting(conn: &Connection, name: &str, default: u32) -> u32 {
    let x: u32 = conn.query_row("SELECT value FROM events WHERE name=?", &[&name], |row| {
        row.get(0)
    })
                     .unwrap_or(default);
    x
...
fn add(conn: &Connection, name: &str) -> Result<()> {
    conn.execute("insert into events(name, delta)
                  select ?, value from \
                  settings where name = 'default_weight';",
                 &[&name])
...
fn choose(conn: &Connection) -> Result<()> {
    ...
    let mut choices: Vec<Weighted<String>> =
        stmt.query_map(&[], |row| {
            Weighted::<String> {
                weight: row.get(1),
                item: row.get(0),
...
fn default_path() -> String {
    ...
    path.push(".options.db");
    std::env::var("OPTIONS_DB")
        .unwrap_or(path.to_str().unwrap_or("./options.db").to_string())
}
...
fn run() -> Result<()> {
    ...
    let app = App::new("Option")
        ...
        .subcommand(SubCommand::with_name("reset")
                        ...
                        .arg(Arg::with_name("weight")
                                 .help("weight to reset all options to")))
        .subcommand(SubCommand::with_name("list")
                        .about("show all defined options"))
        ...
        .subcommand(SubCommand::with_name("more")
                        .about("declare experience with option entry as favourable, increasing \
                                bias for future choices")
                        .arg(Arg::with_name("NAME")
                                 .required(true)
                                 .help("name of option indicate favour"))
                        .arg(Arg::with_name("message").help("message for log entry")))
        .subcommand(SubCommand::with_name("less")
                        .about("declare experience with option entry as unfavourable, decreasing \
                                bias for future choices")
                        .arg(Arg::with_name("NAME")
                                 .required(true)
    ...
    let flags = if cmdline.is_present("create") {
        SQLITE_OPEN_CREATE | SQLITE_OPEN_READ_WRITE
    } else {
        SQLITE_OPEN_READ_WRITE
    };

    let path = Path::new(cmdline.value_of("dbname").unwrap());
    let conn = Connection::open_with_flags(path, flags)
        .chain_err(|| format!("unable to open sqlite db: {:?}", path))?;
    ...
    match cmdline.subcommand() {
        ("add", Some(sub_m)) => {
            add(&conn, sub_m.value_of("NAME").unwrap())
                .chain_err(|| "adding option")?;
            Ok(())
        },
        ("kill", Some(sub_m)) => {
            kill(&conn, sub_m.value_of("NAME").unwrap())
                .chain_err(|| "killing option")?;
            Ok(())
        },
        ("default", Some(sub_m)) => {
            let weight = u32::from_str_radix(sub_m.value_of("WEIGHT").unwrap(), 10)
                .chain_err(|| "converting default weight")?;
            default(&conn, &weight)
                .chain_err(|| "setting default weight")?;
            Ok(())
        },
        ("reset", Some(sub_m)) => {
            let weight = match sub_m.value_of("weight") {
                ...
                Some(w) => {
                    u32::from_str_radix(w, 10)
                        .chain_err(|| "converting reset value")?
                },
            };
        ...
        ("set", Some(sub_m)) => {
            let weight = u32::from_str_radix(sub_m.value_of("WEIGHT").unwrap(), 10)
                .chain_err(|| "converting target weight")?;
            set(&conn,
                sub_m.value_of("NAME").unwrap(),
                sub_m.value_of("message"),
                weight)
